# Makefile
.SUFFIXES:
.SECONDARY:
.PHONY: all clean run
#-------------------------------------------------------------------------------
# ToolChains
#-------------------------------------------------------------------------------
CCompiler := gcc
CppCompiler := g++
Linker := g++

#-------------------------------------------------------------------------------
# Environment
#-------------------------------------------------------------------------------
Msys := /d/msys64/
MinGW := $(Msys)/mingw64
Boost := $(MinGW)/boost

#-------------------------------------------------------------------------------
# Directories
#-------------------------------------------------------------------------------
TopDir := $(CURDIR)

Target := Example

SourceDir := $(TopDir)/Source
IncludeDir := $(TopDir)/Include
BuildDir := $(TopDir)/Build
DistDir := $(TopDir)/Dist

#-------------------------------------------------------------------------------
# Macros
#-------------------------------------------------------------------------------
include $(TopDir)/Tools.mk

#-------------------------------------------------------------------------------
# Files
#-------------------------------------------------------------------------------
BuildObjectDir := $(BuildDir)/Object
BuildDependenceDir := $(BuildDir)/Dependence

CppFile := $(shell find $(SourceDir) -type f -name '*.cpp')
CppRelative := $(shell realpath --relative-to=$(SourceDir) $(CppFile))
BuildObjectCppFile := $(patsubst %.cpp,$(BuildObjectDir)/Cpp/%.o,$(CppRelative))

DistExeFile := $(DistDir)/$(Target).exe

RunAppScript := $(TopDir)/RunApp.sh

#-------------------------------------------------------------------------------
# Libraries
#-------------------------------------------------------------------------------
LibraryEntries := ws2_32
LibraryDirs :=
LibraryIncludeDirs := $(Boost)/include
LibraryDirFlags := $(foreach dir,$(LibraryDirs),-L$(dir))
LibraryFlags := $(foreach entry,$(LibraryEntries),-l$(entry))

#-------------------------------------------------------------------------------
# Includes
#-------------------------------------------------------------------------------
IncludeDirs := $(IncludeDir) $(LibraryIncludeDirs)
IncludeFlags := $(foreach dir,$(IncludeDirs),-I$(dir))

#-------------------------------------------------------------------------------
# Cpp Flags
#-------------------------------------------------------------------------------
CppFlags := $(IncludeFlags) $(LibraryFlags) -Wall -O3 -ffunction-sections -std=c++23

#-------------------------------------------------------------------------------
# Linker Flags
#-------------------------------------------------------------------------------
LinkerScript :=
Specs :=
LinkerFlags := $(LinkerScript) $(Specs) -g

#-------------------------------------------------------------------------------
# Rules
#-------------------------------------------------------------------------------
all: $(DistExeFile)

$(BuildObjectDir)/Cpp/%.o: $(SourceDir)/%.cpp
	@echo $(notdir $<)
	$(call cpp2o,$<,$@,$(BuildDependenceDir)/$*.d,$(CppFlags))

$(DistDir)/%.exe: $(BuildObjectCppFile)
	@echo linking ... $(notdir $@)
	@$(call o2exe,$^,$@,$(LinkerFlags),$(LibraryDirFlags),$(LibraryFlags),$(BuildDir)/$*.map)

-include $(BuildDependenceDir)/*.d

clean:
	@echo clean ...
	@rm -rf $(BuildDir) $(DistDir)

run: $(DistExeFile)
	@echo running ... $(notdir $<)
	@$(RunAppScript) $<